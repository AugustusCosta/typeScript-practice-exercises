...
function composeFunctions(...functions: Array<(input: any) => any>) {
    return (input: any) =>
      functions.reduceRight((currentValue, currentFunction) => currentFunction(currentValue), input);
  }
   ...
   */
  function convertStringToCharacterArray(text: string): string[] {
    return text.split('');
  }
   ...
   */
  function reverseCharacterArray(characters: string[]): string[] {
   ...
   */
  function joinCharacterArrayToString(characters: string[]): string {
    return characters.join('');
  }
  
  // Compondo as funções para criar a função final que inverte a string
  const reverseString = composeFunctions(
    joinCharacterArrayToString,   // Recebe string[] e retorna string
